...
async def get_stamp_info(data: StampRequestData):
    ...
    # 1. フロントエンドが "glasses_touka.png" と送っても "glasses_touka" と送っても対応
    #    まず .png を取り除いてベース名 (例: "glasses_touka") を取得
    base_stamp_id = data.stamp_id.replace(".png", "")
    
    # 2. 辞書検索用のキー (例: "glasses_touka.png") を作成
    config_key = base_stamp_id + ".png"

    # 3. ファイルパス (例: "www/glasses_touka.png") を作成
    stamp_path = os.path.join(WWW_DIR, config_key)
    
    if not os.path.exists(stamp_path):
        raise HTTPException(
            status_code=404,
            detail=f"Stamp asset '{config_key}' not found on server."
        )
# 2) スタンプ画像ファイルを www/<stamp_id> から読む
    ...
    if stamp_type == "glasses":
        ...
        x = -200 #(left_eye_landmark["x"] + right_eye_landmark["x"]) // 2
        y = -200 #(left_eye_landmark["y"] + right_eye_landmark["y"]) // 2
